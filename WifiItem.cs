...
public class WifiInfoItem : IEquatable<WifiInfoItem>
{
    ...
    [JsonInclude]
    public float _Distance { get; private set; } = 0f;

    [JsonIgnore]
    public string Distance
    {
        get => $"{_Distance.ToString("F2")}m";
        set => _Distance = float.Parse(value);
    }

    [JsonInclude]
    public float PrimaryFrequency { get; set; } = 0f;

    public static float DistanceCalc(float _Frequency, float _RSSI)
    {
        //10^((27.55 - 20*log10(f) + |R|)/20)
        return (float)Math.Pow(10, (27.55d - 20 * Math.Log10(_Frequency) + Math.Abs(_RSSI)) / 20d);
    }
